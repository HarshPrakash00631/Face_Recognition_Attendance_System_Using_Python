...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'hp00631@gmail.com' ")
...
def TrackImages():
    ...
    xlsx_path = "Attendance\Attendance_" + date + ".xlsx"
    
    already_marked = []
    attendance_data = []

    # NEW: Load existing records from Excel if the file exists
    if os.path.exists(xlsx_path):
        existing_df = pd.read_excel(xlsx_path)
        already_marked = existing_df['Name'].tolist()
        # Fill the GUI table with existing records
        for index, row in existing_df.iterrows():
            tv.insert('', 0, text=str(row['Id']), values=(str(row['Name']), str(row['Date']), str(row['Time'])))
            attendance_data.append([str(row['Id']), '', str(row['Name']), '', str(row['Date']), '', str(row['Time'])])
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                if bb not in already_marked:
                    curr_ts = time.time()
                    curr_date = datetime.datetime.fromtimestamp(curr_ts).strftime('%d-%m-%Y')
                    curr_time = datetime.datetime.fromtimestamp(curr_ts).strftime('%H:%M:%S')
                    
                    # Add to local list
                    new_entry = [str(ID), '', bb, '', curr_date, '', curr_time]
                    attendance_data.append(new_entry)
                    already_marked.append(bb)
                    
                    # Update GUI
                    tv.insert('', 0, text=str(ID), values=(bb, curr_date, curr_time))
                    
                    cv2.putText(im, f"Marked: {bb}", (x, y-10), font, 1, (0, 255, 0), 2)
                    cv2.imshow('Taking Attendance', im)
                    cv2.waitKey(1)
                    
                    engine.say(f"Attendance marked for {bb}")
                    engine.runAndWait()
            else:
    ...
    # SAVE TO EXCEL WITH AUTO-ADJUST
    if attendance_data:
        # Create DataFrame from the list (excluding the empty spacer columns for Excel)
        # We filter the columns to only include the actual data for the Excel file
        clean_data = [[row[0], row[2], row[4], row[6]] for row in attendance_data]
        final_df = pd.DataFrame(clean_data, columns=['Id', 'Name', 'Date', 'Time'])
